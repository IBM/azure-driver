...
PKG_ROOT = 'azuredriver'
PKG_INFO = 'pkg_info.json'
DIST_DIR = 'dist'
WHL_FORMAT = 'azuredriver-{version}-py3-none-any.whl'

DOCS_FORMAT = 'azuredriver-{version}-docs'
DOCS_DIR = 'docs'

DOCKER_WHLS_DIR = 'whls'
DOCKER_PATH = 'docker'
DOCKER_IMG_NAME = 'azure-driver'
DOCKER_REGISTRY = 'icr.io/cp4na-drivers'

HELM_CHART_PATH = os.path.join('helm', 'azure-driver')
HELM_CHART_NAME = 'azure-driver'
HELM_CHART_NAME_FORMAT = 'azure-driver-{0}.tgz'
...
class Builder:
    ...
    def convert_bicep_files(self):
        with self.stage('Generating JSON from Bicep') as s:
            bicep_path = os.path.join(self.project_path, PKG_ROOT, 'config')
            json_path = os.path.join(bicep_path, 'json')
            for file in os.listdir(bicep_path):
                if file.endswith(".bicep"):
                    s.run_cmd('az', 'bicep', 'build', '--file', os.path.join(bicep_path, file), '--outdir', json_path)
